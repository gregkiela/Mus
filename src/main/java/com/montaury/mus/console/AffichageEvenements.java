...
public class  AffichageEvenements implements Evenements {
  ...
  public void mancheTerminee(Partie.Score score) {
    ...
    score.resultatManches().forEach(manche -> afficher("Vainqueur : " + manche.vainqueur().nomEquipe() + ", score du perdant : " + manche.pointsVaincu()));
  }
  ...
  public void tourTermine(Opposants opposants, Manche.Score score) {
    ...
    opposants.dansLOrdreJoueur().forEach(this::afficherMain);
    score.scoreParEquipe().forEach((key, value) -> afficher("Score " + key.nomEquipe() + ": " + value));
    sautLigne();
  }
